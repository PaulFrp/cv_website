export const nasDetail = {
	...
	overview:[
		...
		"The motivation behinf this project was to self host containerized applications rather than relying on cloud services that have complete control over my data.",
		" It was also my first time deploying a full project via docker. I used docker-compose files split like this: a full *arr stack (Radarr/Sonarr) paired with Jellyfin for media management and streaming, qBittorrent for downloads routed through Gluetun (Mullvad VPN over WireGuard) to keep torrent traffic secured, Caddy as a reverse proxy for clean routing to services, Vaultwarden (self-hosted Bitwarden) for password management, and Uptime Kuma for monitoring service health. ",
		"For remote access, I used Tailscale to reach Jellyfin and Vaultwarden securely from outside the home network including getting Jellyfin working across various TV platforms without exposing ports directly to the internet.", 
	],
	techStack: [
	...
	challenges: [
		...
		},
		{
			...
			solution: [
				...
				"This was also my first realisation of all the possible ways docker could be used. On the same machine I was able to have a whole container under a VPN while some other containers were on the tailscale network. I was scared of running into IP problems for conections but thanks to docker it was well handled",
			],
		},
		{
			title: "qBittorrent crash on boot (VPN race condition)",
			problem:[
				"Once (after a 2 weeks holliday) I had to reboot my NAS for some reason (:D). It would not start up so I investigated docker logs and found at that qBittorrent container tried starting but failed for no apparent reasons. More digging led me to find that the container was trying to start before the Gluetun VPN container had a healthy WireGuard tunnel.",
				"For this reason the conatiner would come up without a working VPN path and due to specific guardrails that I created it failed to boot, leaving the download stack broken until a manual restart.",
			],
			solution: [
				"To solve this issue, I added a Docker healthcheck on the Gluetun container so Compose can detect when the VPN tunnel is actually up",
	...
	lessonsLearned:
		"A Compose file that lists services is not enough, 'depends_on' without a properly set up health condition could still create races on reboot. Having Gluetun healthchecks with 'service_healthy' dependencies made the whole stack properly start up on cold boots. For remote access, Tailscale was a cleaner security model than port-forwarding: private connection to Jellyfin and Vaultwarden without putting those services on the public internet.",
};
